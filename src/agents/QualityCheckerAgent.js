...
/**
 * 质量检查系统提示词：要求AI进行深度审查，但评分需结合客观工具数据。
 */
...
/**
 * 工具执行器：封装外部编译/测试工具的调用。
 */
class ToolRunner {
  constructor() {
    this.toolCache = new Map(); // 缓存工具可用性
  }
  ...
  /**
   * 安全执行外部命令，返回 { exitCode, stdout, stderr }。
   */
  ...
  compileCode(code, language, options = {}) {
    ...
    if (language === 'c') {
      if (this.hasTool('gcc')) {
        ...
        result = { compiled: exec.exitCode === 0, errors: exec.stderr, warnings: exec.stdout, executable: exec.exitCode === 0 ? outFile : null };
  ...
  lintCode(code, language, options = {}) {
    ...
    if (language === 'python') {
      ...
      // C/C++ 的编译检查已经包含 -Wall -Wextra，这里不做额外 lint
...
class QualityCheckerAgent extends BaseAgent {
  constructor(provider, options = {}) {
    ...
    this.minQualityScore = options.minQualityScore || 75; // 改动1：阈值从60提升到75
  }
  ...
  async checkQuality(task, result, context = {}) {
    ...
    if (this.enableCompilation && codeInfo) {
      compileResult = this.toolRunner.compileCode(code, language);
    }
    if (this.enableLint && codeInfo) {
      lintResult = this.toolRunner.lintCode(code, language);
    }
    if (this.enableTest && codeInfo) {
      testResult = this.toolRunner.runTests(code, language);
    }

    const staticCheckResults = this._staticCodeCheck(code, language, context.constraints);
    ...
    prompt += `\n【客观工具验证结果】\n`;
    if (compileResult) {
      prompt += `编译检查: ${compileResult.compiled ? '✅ 通过' : '❌ 失败'}\n`;
      if (compileResult.errors) prompt += `编译错误: ${compileResult.errors.substring(0, 500)}\n`;
      if (compileResult.warnings) prompt += `编译警告: ${compileResult.warnings.substring(0, 500)}\n`;
    }
    if (lintResult) {
      prompt += `静态检查: ${lintResult.passed ? '✅ 通过' : '⚠️ 有问题'} (${lintResult.tool})\n`;
      if (lintResult.issues.length > 0) {
        prompt += `问题列表: ${lintResult.issues.slice(0, 5).join('; ')}\n`;
      }
    }
    if (testResult) {
      prompt += `测试执行: ${testResult.run ? '✅ 已执行' : '❌ 未执行'} (${testResult.tool})\n`;
      prompt += `通过/失败: ${testResult.passed}/${testResult.failed}\n`;
    }

    if (staticCheckResults.constraintViolations.length > 0) {
      prompt += `\n【约束违规】\n${staticCheckResults.constraintViolations.map(v => `- ${v}`).join('\n')}\n`;
    }
    if (staticCheckResults.securityIssues.length > 0) {
      prompt += `\n【安全问题】\n${staticCheckResults.securityIssues.map(v => `- ${v}`).join('\n')}\n`;
    }

    prompt += `\n【代码指标】\n`;
    prompt += `  行数: ${staticCheckResults.metrics.linesOfCode}\n`;
    prompt += `  注释率: ${staticCheckResults.metrics.commentRatio}%\n`;
    prompt += `  函数数: ${staticCheckResults.metrics.functionCount}\n`;
    ...
    if (context.isFinalReview === true) {
      prompt += `\n\n【最终审查模式】
本次审查的是**多个子任务合并后的整体产出**。
请额外关注以下维度：
1. 跨文件一致性：import/函数调用是否对应，接口是否匹配
2. 整体项目结构完整性：是否缺少必要的入口文件、配置文件等
3. 子任务产出之间是否有接口断裂：如某个模块调用了未定义的函数
4. 是否有重复/冲突的定义：如同一函数在多个文件中被定义
5. 整体代码风格是否统一

在评分时，如果发现上述问题，请额外扣分（每项-10分）。`;
    }

    const response = await this.sendOnce(prompt);
    const parsed = this._extractJson(response.content);

    // ════════════════════════ 3. 客观评分修正 ════════════════════════
    const finalResult = this._applyObjectiveScoring(
      parsed, task, compileResult, lintResult, testResult, staticCheckResults
    );
  ...
  _applyObjectiveScoring(parsed, task, compileResult, lintResult, testResult, staticCheckResults) {
    const base = parsed || {
      taskId: task.id, qualityScore: 60, status: 'completed',
      strengths: ['任务基本完成'], weaknesses: ['无法自动评估详细质量'],
      nextSteps: ['建议人工确认'], revisionSuggestions: '', canProceed: true,
      constraintViolations: [], securityIssues: [], codeMetrics: {}
    };
    ...
    return {
      ...
      rawResponse: base.rawResponse || null
    };
  }
  ...
  _calculateMetrics(code) {
    ...
    let commentLines = 0, codeLines = 0, inBlockComment = false;
