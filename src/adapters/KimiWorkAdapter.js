...
class KimiWorkAdapter extends BaseToolAdapter {
  constructor (options = {}) {
    super({
      name: 'kimiwork',
      displayName: 'KimiWork',
      description: 'KimiWork AI Agent - 智能编程协作平台',
      command: 'kimi-work',
      ...options
    });
  }
  ...
  async detect () {
    ...
    try {
      const cmdPath = await this._findCommandInPath('kimi-work');
      if (cmdPath) {
      ...
      const cmdPath2 = await this._findCommandInPath('kimiwork');
      if (cmdPath2) {
        this.installPath = cmdPath2;
        this.command = 'kimiwork';
        this.detected = true;
      ...
      const possiblePaths = [
        path.join(localAppData, 'Programs', 'KimiWork', 'kimi-work.exe'),
        path.join(localAppData, 'KimiWork', 'kimi-work.exe'),
        path.join(process.env.APPDATA || '', 'KimiWork', 'kimi-work.exe'),
        path.join('C:', 'Program Files', 'KimiWork', 'kimi-work.exe'),
        path.join('C:', 'Program Files (x86)', 'KimiWork', 'kimi-work.exe'),
        path.join(process.env.HOME || '', 'kimiwork', 'bin', 'kimi-work'),
        path.join('/usr', 'local', 'bin', 'kimi-work')
      ];
  ...
  async connect (_options = {}) {
    ...
    if (!this.detected) {
      throw new Error('KimiWork 未安装或未找到');
    }
    ...
    try {
      ...
      if (result.success) {
        this.status = 'online';
        return { success: true, message: 'KimiWork 连接成功' };
      }
      this.status = 'offline';
      return { success: false, message: 'KimiWork 不可用' };
    } catch (e) {
      this.status = 'error';
  ...
  async execute (task, options = {}) {
    ...
    const outputDir = options.outputDir || `./workspace/kimiwork/${taskId}`;
    ...
    const result = await this._runCommand(this.command, ['task', '--input', task], {
...
module.exports = KimiWorkAdapter;
